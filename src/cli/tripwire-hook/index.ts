// FORGE-202 follow-on: `forge tripwire-hook` — the FIRST live consumer of the
// Tripwire scanner. Wired as a Claude Code PostToolUse hook, it reads the host's
// PostToolUse JSON from stdin, scans the (untrusted) tool/MCP output for
// prompt-injection, and — only on a hit — emits a CONSTANTS-ONLY warning back to
// the model via `additionalContext`. Clean output → silent exit 0.
//
// THREAT MODEL (see spec/THREAT-MODEL.md):
//   - PostToolUse runs AFTER the tool already executed, so this can only WARN,
//     never BLOCK. It is report-only by construction.
//   - The warning is itself a model-visible prompt sink. It is therefore built
//     from CONSTANTS ONLY: the severity word + the matched rule IDs (from the
//     fixed TripwireRuleId enum) + a fixed instruction sentence. It NEVER echoes
//     any substring of tool_response/tool_input — no excerpts (even redacted),
//     no URLs/titles/domains, no raw tool_name, no parse/error text. Re-injecting
...
//     must never destabilize the user's Claude Code session. Never throw, never
//     exit 2, never log raw stdin/output/errors.

import { scanText, type TripwireSeverity, type TripwireSource } from '../../security/tripwire/scan.ts';
import type { TripwireRuleId } from '../../security/tripwire/rules.ts';

export interface TripwireHookOptions {
  ...
  return new Promise((resolve) => {
    ...
    stream.on('data', (chunk: Buffer | string) => {
      ...
      if (total > STDIN_MAX_BYTES) {
        // Over the cap — stop accumulating and fail open.
        try {
          stream.pause();
        } catch {
          // ignore
...
function collectStrings(value: unknown, depth: number, acc: string[], charCount: number): number {
  ...
  if (value !== null && typeof value === 'object') {
    ...
    // Priority fields first, then the rest (each visited once).
    const ordered = [
...
function extractText(toolResponse: unknown): string {
  ...
  // Bounded fallback — no string leaves (e.g. all-numeric structured output).
  try {
...
function buildWarning(severity: TripwireSeverity, ruleIds: readonly TripwireRuleId[]): string {
  ...
  return (
    `Forge Tripwire flagged the tool output you just received as a possible ` +
 ...
 * the caller in a never-throw fail-open guard.
 */
async function run(opts: TripwireHookOptions): Promise<void> {
  ...
  // Dedup matched rule IDs in stable enum-discovery order.
 ...
 */
export async function runTripwireHook(opts: TripwireHookOptions = {}): Promise<void> {
  try {
    await run(opts);
  } catch {
...
/**
 * Bin entry point. Always resolves to exit code 0 (fail-open). Returned (not
 * process.exit'd) so the bin wiring and tests share one path.
 */
export async function tripwireHookMain(opts: TripwireHookOptions = {}): Promise<number> {
  await runTripwireHook(opts);
  return 0;
}
