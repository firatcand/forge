...
//   Q2 headings to strip → 5 (drop ## Critical paths; keep ## Branch strategy)
 ...
 * Returns the trimmed string with 3+ consecutive newlines collapsed to 2.
 */
...
export async function migrateClaudemd(opts: MigrateOptions): Promise<MigrateResult> {
  ...
  // Precondition 1: refuse if already migrated.
  if (existsSync(contextPath)) {
    return {
      exitCode: 1,
      filesChanged: [],
      stderr:
        'forge upgrade --migrate-claudemd: .forge/CONTEXT.md already exists. Migration is one-shot; this repo appears to already be migrated.',
    };
  }

  // Precondition 2: CLAUDE.md must exist.
  ...
  // Precondition 3: settings.yaml must exist + parse (spec §9 exit 3).
  // Post-migration state requires settings.yaml, so refuse upfront if it's
  // missing. The user runs `forge init` first.
  ...
  return {
    ...
    stderr: [
      ...
      '  Original saved to CLAUDE.md.pre-migration.bak.',
      '  Commit CLAUDE.md, .gitignore, and the new product-only template;',
      '  .forge/CONTEXT.md is gitignored (other devs run `forge upgrade` after pulling).',
    ].join('\n'),
  };
}
