...
test('recall caps symbols per touched file', async () => {
  ...
  try {
    ...
    backend.upsertEdges(edges);
    const hits = backend.recallForTask('A', { limit: 1000 });
...
test('recall drops a corrupt (over-bound) symbol row instead of surfacing it', async () => {
  ...
  try {
    ...
    backend.upsertEdges([{ src: 'task:A', dst: 'file:src/a.ts', kind: 'touches' }]);
...
test('query filters by id, kind, and escaped title substring', async () => {
  ...
  try {
    ...
    // exact id
    assert.deepEqual(backend.query({ id: 'task:A' }).map((n) => n.id), ['task:A']);
    // kind filter (sorted by id)
    assert.deepEqual(backend.query({ kind: 'task' }).map((n) => n.id), ['task:A', 'task:B']);
    ...
    const pct = backend.query({ title: '%' });
    assert.deepEqual(pct.map((n) => n.id), ['task:B'], 'wildcard % must be escaped to a literal');
    // case-insensitive
    assert.deepEqual(backend.query({ title: 'lesson' }).map((n) => n.id), ['learning:l1']);
    // limit clamps
    assert.equal(backend.query({ kind: 'task', limit: 1 }).length, 1);
  } finally {
    backend.close();
    cleanup();
  }
...
test('query fails loud on a corrupt node row', async () => {
  ...
  try {
    ...
    assert.throws(() => backend.query({ id: 'weird:1' }), /invalid|enum|kind/i);
  } finally {
    backend.close();
    cleanup();
  }
...
test('traverse returns a bounded subgraph honoring depth and excluding dangling edges', async () => {
  ...
  try {
    ...
    const d1 = backend.traverse('task:A', { depth: 1 });
...
test('traverse caps nodes and flags truncation', async () => {
  ...
  try {
    ...
    backend.upsertNodes(nodes);
    backend.upsertEdges(edges);
    const sub = backend.traverse('task:hub', { depth: 1, limit: 3 });
...
test('traverse counts DISTINCT neighbors, not raw edge rows (parallel edge kinds)', async () => {
  ...
  try {
    ...
    backend.upsertNodes(nodes);
    backend.upsertEdges(edges);
    const sub = backend.traverse('task:root', { depth: 1, limit: 200 });
...
test('traverse returns empty for an absent node', async () => {
  ...
  const backend = await openLocalBackend(dbPath);
  try {
    backend.upsertNodes([{ id: 'task:A', kind: 'task', title: 'A', body: '' }]);
    const sub = backend.traverse('task:nope');
...
test('doctor on a clean graph reports no problems', async () => {
  ...
  try {
    ...
    backend.upsertEdges([{ src: 'task:A', dst: 'task:B', kind: 'depends_on' }]);
    const health = backend.doctor();
