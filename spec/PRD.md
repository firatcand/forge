...
- [ ] **Static settings validation** rejects `ship.merge_policy: 'auto'` when dual-host review is not configured (single-host); the supported-RepoHost requirement (non-GitHub remote) is enforced **at runtime** — the honesty probe detects it and parks the task with a question, exactly like any other probe failure (fail-closed — never warn-and-merge, never silent downgrade)
...
> **Re-scoped 2026-06-11.** — `/reconcile` shipped in v0.4 via FORGE-100; `/amend-roadmap` **shipped in v0.4 via FORGE-101** with two amendments to the original design: (1) the write path is **tracker-first** (issue + relations on the tracker, then materialized into phases.yaml through the reconcile `--pull` staged-addition path — no dual-write rollback; phases.yaml stays single-writer per SPEC §Source of truth), journaled + resumable under `.forge/orchestrator/global/amend-journal/`; (2) `worktree-drift-guard` stays dropped (FORGE-103 canceled per the §21 pivot) — its role is reduced to an **inline drift warning**: after a successful amend, the verb lists active attempts (claimed/dispatched/running/blocked_on_question/awaiting_respawn) so the supervisor knows which in-flight worktrees now run against a mutated roadmap. Informational only, no rebase/restart workflow.
...
- **`/amend-roadmap`** *(shipped v0.4 — FORGE-101)* — create new tasks mid-flight; tracker-first: creates the issue + dependency relations on the tracker, then materializes the task into phases.yaml via the reconcile pull path (resumable journal; a half-applied amend is always tracker-ahead, which `/reconcile --pull` already understands)
...
**Acceptance criteria**
- [x] `/amend-roadmap` is tracker-first: issue + relations on the tracker, then phases.yaml via the reconcile pull staged-addition path; every step journaled + resumable (`--resume <task-id>`), crash-after-create adopts instead of duplicating (FORGE-101)
- [x] New task gets stable `task_id` (e.g., P2.5-T<next>); preserves existing IDs; the O_EXCL journal doubles as the id reservation against concurrent amends (FORGE-101)
...
- [x] Inline drift warning (replaces the dropped guard): after a successful amend, list active attempts whose worktrees now run against a mutated roadmap (FORGE-101)

**Edge cases / failure modes**
| Edge case | Expected behavior |
|---|---|
| `/amend-roadmap` fails mid-flight (tracker or local) | Journal records the failed step; phases.yaml untouched until the final materialize step; user re-runs with `--resume <task-id>` (tracker-created issue is adopted, never duplicated) |
...
> /amend-roadmap                       [creates new task: tracker-first, then phases.yaml via reconcile]
