...
`id` is computed by the SINGLE append helper, never by producers: progress events use the time-free natural key `<task_id>:<state_version>:<type>` (the state CAS makes `state_version` unique per transition), question events `<task_id>:<question_id>:<type>`; fatal `<task_id ?? run_id>:<epoch-ms>:fatal`, EXCEPT keyed fatals (retry exhaustion) which carry `details.failure_key` and get the time-free `<task_id>:<failure_key>:fatal` so a crash-replayed re-emission dedups. It is optional on the READ union so legacy files parse; required at write time. Readers (attach) dedup on it.
...
**Discovery/auth is independent of `tracker.type`:** the GitHub RepoHost activates on a **github.com** remote + authenticated `gh` (`gh auth status --hostname github.com`), regardless of tracker. GitHub Enterprise hosts are out of v0.4 scope (owner decision, FORGE-232: the persisted identity model is host-less `owner/repo`; GHES support requires a versioned schema extension) — a GHES remote gets no RepoHost, same as any non-GitHub remote. Once a task's ship record holds a persisted base, that durable identity is authoritative for recovery: the adapter is constructed from the record without re-sniffing remotes, so a remote swapped away after PR creation can still reconcile its recorded GitHub PR. Fork topologies (base repo ≠ push repo) are detected and **parked** — out of scope for v0.4. Non-GitHub remotes have no RepoHost, so **orchestrator SHIP is unavailable there under BOTH policies**: a `reviewed` task on such a repo parks with a question (Phase 3 requires RepoHost PR creation and `mergeResult()` merge proof — without them the task could never legitimately reach `shipped`); the interactive `/ship` skill remains the manual path. `ship.merge_policy: 'auto'` on such a repo is additionally a settings validation error.
...
- `mergeAtomic(pr, expectedHeadSha)` — the `'auto'` merge step: squash merge with the server-side expected-head check (`gh pr merge --squash --match-head-commit`); fails — never bypasses — if the head moved or protections are unsatisfied. No standing auto-merge enablement is ever *intended*; because `gh pr merge` silently enrolls into a merge queue / auto-merge when repo policy demands it, the adapter runs **compensating cleanup** (FORGE-232, owner decision 2026-07-23): a pre-merge re-probe, then bounded post-command GraphQL observation (`autoMergeRequest`, `isInMergeQueue`, `mergeQueueEntry`) and revocation by form (`--disable-auto` for auto-merge; `dequeuePullRequest` for queue entries) with bounded confirmation. This is mitigation, not an invariant: a crash between enrollment and cleanup, or an enrollment that merges before revocation, remains an acknowledged residual race (`merge_pending` reconciliation + gc own the recovery). A merged result whose head differs from `expectedHeadSha` is reported as `tainted_merge` — never success; a closed-unmerged PR reports `pr_closed`.
